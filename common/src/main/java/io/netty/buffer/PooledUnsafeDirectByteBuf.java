...
final class PooledUnsafeDirectByteBuf extends PooledByteBuf<ByteBuffer> {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(PooledUnsafeDirectByteBuf.class);
  ...
  private PooledUnsafeDirectByteBuf(
      Handle<PooledUnsafeDirectByteBuf> recyclerHandle, int maxCapacity) {
    super(recyclerHandle, maxCapacity);
    LOG.warn("Check PooledUnsafeDirectByteBuf constructor");
    LOG.warn(Thread.currentThread().getName());
    LOG.warn(this.toString());
    LOG.warn(this.getClass().getName() + "@" + Integer.toHexString(System.identityHashCode(this)));
    LOG.warn("size: " + this.readableBytes());
    LOG.warn("refCnt: " + this.refCnt());
  }
  ...
  public boolean release() {
    LOG.warn("Check PooledUnsafeDirectByteBuf release empty");
    LOG.warn(Thread.currentThread().getName());
    LOG.warn(this.toString());
    try {
      LOG.warn(
    ...
    LOG.warn("size: " + this.readableBytes());
    LOG.warn("refCnt: " + this.refCnt());
    LOG.warn("Check PooledUnsafeDirectByteBuf release stack tree", new Throwable());
    return super.release();
  }

  @Override
  public boolean release(int decrement) {
    LOG.warn("Check PooledUnsafeDirectByteBuf release with decrement");
    LOG.warn(Thread.currentThread().getName());
    LOG.warn(this.toString());
    try {
      LOG.warn(
